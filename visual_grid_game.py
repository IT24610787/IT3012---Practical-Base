...
class VisualGridHuntGame:
    ...
    def __init__(self, width=10, height=10, num_food=10, num_opponents=2, num_traps=0, custom_walls=None):
        self.width = width
        self.height = height
    ...
    def get_percept(self) -> dict:
        #modified by IT24610787
        ax, ay = self.agent_pos

        wall_ahead = (
            (ax + 1 >= self.width) or 
            (ay + 1 >= self.height) or 
            ((ax + 1, ay) in self.walls)
        )

        return {
            'agent_pos': list(self.agent_pos),   # Lab 3: the search needs a start state
            'wall_ahead': wall_ahead,
            'food_here': tuple(self.agent_pos) in self.food_positions,   # was 'food here'
        ...
        #     'smells_toxin': tuple(self.agent_pos) in self.toxic_traps,  # Added by IT24610787  
        # }
    ...
    def run_loop(self):
        ...
        def step():
            if not self.env.is_done():
                ...
                self.draw_grid()
                self.label.config(
                    text=f"[{self.agent.active_algo}] Score: {self.env.score} | Steps: {self.env.steps} | Action: {action}")
